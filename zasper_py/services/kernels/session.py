...
from zasper_backend.utils.adapter import adapt
from zasper_backend.utils.jsonutil import json_clean, squash_dates, json_default, extract_dates
...
class Session:
    ...
    session = ""  # The UUID identifying this session.
    ...
    # CBytes(config=True, help="""execution key, for signing messages."""))
    ...
    def _signature_scheme_changed(self, change: t.Any) -> None:
        ...
        if not new.startswith("hmac-"):
            raise ValueError("signature_scheme must start with 'hmac-', got %r" % new)
        hash_name = new.split("-", 1)[1]
        try:
            self.digest_mod = getattr(hashlib, hash_name)
        except AttributeError as e:
            raise ValueError("hashlib has no such attribute: %s" % hash_name) from e
        self._new_auth()
    ...
    keyfile = ""  # path to file containing execution key.
    ...
    pack = default_packer  # the actual packer function
    ...
    unpack = default_unpacker  # the actual packer function
    ...
    def __init__(self, **kwargs: t.Any) -> None:
        ...
        self._key = self._key_default()
        self.digest_mod = self._digest_mod_default()
