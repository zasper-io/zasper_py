...
from zasper_backend.services.websocketHandler.connection.base import BaseKernelWebsocketConnection, \
    serialize_binary_message, serialize_msg_to_ws_v1
import typing as t
...
client_protocol_version = "%i.%i" % protocol_version_info
...
class ZMQChannelsWebsocketConnection(BaseKernelWebsocketConnection):
    ...
    # limited."""
    iopub_msg_rate_limit = 1000
    # """(bytes/sec)
    # Maximum rate at which stream output can be sent on iopub before they are
    # limited."""
    iopub_data_rate_limit = 1000000
    ...
    _open_sessions = {}
    _open_sockets = weakref.WeakSet()

    _kernel_info_future: Future[t.Any]
    _close_future: Future[t.Any]

    channels = {}
    kernel_info_channel = None

    _kernel_info_future = None  # Instance(klass=Future)  # type:ignore[assignment]

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._kernel_info_future = self._default_kernel_info_future()
        self._close_future = self._default_close_future()
        self.parent = kwargs['parent']
        self.websocket_handler = kwargs['websocket_handler']

    def _default_kernel_info_future(self):
        """The default kernel info future."""
        return Future()

    _close_future = None  # Instance(klass=Future)  # type:ignore[assignment]
    ...
    _iopub_msgs_exceeded = False
    _iopub_data_exceeded = False
    ...
    _iopub_window_byte_queue: t.List[t.Any] = []
    ...
    def nudge(self):
        ...
        if getattr(self.kernel_manager, "execution_state", None) == "busy":
            logger.debug("Nudge: not nudging busy kernel %s", self.kernel_id)
        ...
        def on_shell_reply(msg):
            ...
            logger.debug("Nudge: shell info reply received: %s", self.kernel_id)
            if not info_future.done():
                logger.debug("Nudge: resolving shell future: %s", self.kernel_id)
                info_future.set_result(None)

        def on_control_reply(msg):
            """Handle nudge control replies."""
            logger.debug("Nudge: control info reply received: %s", self.kernel_id)
            if not info_future.done():
                logger.debug("Nudge: resolving control future: %s", self.kernel_id)
                info_future.set_result(None)

        def on_iopub(msg):
            """Handle nudge iopub replies."""
            logger.debug("Nudge: IOPub received: %s", self.kernel_id)
            if not iopub_future.done():
                iopub_channel.stop_on_recv()
                logger.debug("Nudge: resolving iopub future: %s", self.kernel_id)
        ...
        def nudge(count):
            ...
            if self.kernel_id not in self.multi_kernel_manager:
                logger.debug("Nudge: cancelling on stopped kernel: %s", self.kernel_id)
                finish()
                return

            # check for closed zmq socket
            if shell_channel.closed():
                logger.debug("Nudge: cancelling on closed zmq socket: %s", self.kernel_id)
                finish()
                return
            ...
            if not both_done.done():
                log = logger.warning if count % 10 == 0 else logger.debug
    ...
    async def _register_session(self):
        ...
        if stale_handler:
            logger.warning("Replacing stale connection: %s", self.session_key)
            stale_handler.close()
        if (
    ...
    async def prepare(self):
        ...
        print("==========session_key for kernel_manager===========")
        print(self.kernel_manager.session.key)
        ...
        def give_up():
            ...
            if future.done():
                return
            logger.warning("Timeout waiting for kernel_info reply from %s", self.kernel_id)
            future.set_result({})
    ...
    def connect(self):
        ...
        if buffer_info and buffer_info["session_key"] == self.session_key:
            logger.info("Restoring connection for %s", self.session_key)
            ...
            def replay(value):
                ...
                if replay_buffer:
                    logger.info("Replaying %s buffered messages", len(replay_buffer))
            ...
            try:
                ...
                try:
                    ...
                    logger.error("Error opening stream: %s", e)
                except KeyError:
                    pass
    ...
    def disconnect(self):
        """Handle a disconnect."""
        logger.debug("Websocket closed %s", self.session_key)
    ...
    def handle_incoming_message(self, incoming_msg: str) -> None:
        ...
        if not self.channels:
            ...
            logger.debug("Received message on closed websocket %r", ws_msg)
            return
        ...
        if channel is None:
            logger.warning("No channel specified, assuming shell: %s", msg)
            channel = "shell"
        if channel not in self.channels:
            logger.warning("No such channel: %r", channel)
            return
    ...
    def _on_zmq_reply(self, stream, msg_list):
        ...
        if stream.closed():
            logger.warning("zmq message arrived on closed channel")
            self.disconnect()
            return
        channel = getattr(stream, "channel", None)
        if self.subprotocol == "v1.kernel.websocket.jupyter.org":
            ...
            try:
                ...
                logger.critical("Malformed message: %r" % msg_list, exc_info=True)
            else:
                try:
    ...
    def request_kernel_info(self):
        ...
        try:
            ...
            logger.debug("Requesting kernel info from %s", self.kernel_id)
            ...
            if not future.done():
                logger.debug("Waiting for pending kernel_info request")
    ...
    def _handle_kernel_info_reply(self, msg):
        ...
        try:
            ...
            logger.error("Bad kernel_info reply", exc_info=True)
            self._kernel_info_future.set_result({})
            return
        else:
            info = msg["content"]
            logger.debug("Received kernel info: %s", info)
            if msg["msg_type"] != "kernel_info_reply" or "protocol_version" not in info:
                logger.error("Kernel info request failed, assuming current %s", info)
                info = {}
        ...
        if msg_type not in {"status", "comm_open", "execute_input"}:
            ...
            # resume once we've got some headroom below the limit
                ...
                if not self._iopub_data_exceeded:
                    logger.warning("iopub messages resumed")
    ...
    def on_kernel_restarted(self):
        ...
        logger.warning("kernel %s restarted", self.kernel_id)
        self._send_status_message("restarting")

    def on_restart_failed(self):
        """Handle a kernel restart failure."""
        logger.error("kernel %s restarted failed!", self.kernel_id)
